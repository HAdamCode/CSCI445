...
function getNumberOfBooks($conn, $product_id)
{
...
function getCustomersBySearch( $conn, $input, $searchBy) {
    if ($searchBy === 'first') {
        $sql = "SELECT * FROM customer WHERE first_name LIKE '$input%'";
    } elseif ($searchBy === 'last') {
        $sql = "SELECT * FROM customer WHERE last_name LIKE '$input%'";
    }
    $result = $conn->query($sql);
    return $result;
}
